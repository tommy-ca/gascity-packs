...
- Disposable live-city formula list, formula show, and agent listing when `GC_TEST_BIN` is set; no canonical city mutation. Formula sling of `pstack-poteto-mode` and `pstack-build` is proven as cook plus route by `scripts/pstack_host_sling_proof.py`.
- `python3 scripts/gascity_pack_inference_gate.py --pack pstack --setup-only --skip-inference-env-check` compiles `pstack-review` and `pstack-build` with `gc formula show` in the inference-gate city. That city is not formula sling.
- `gc pack registry publish --dry-run pstack/` is proven from this branch. `gc pack registry whoami` succeeds. Dry-run is not registry acceptance. Hosted identity is `tommy-ca/pstack`. Unscoped hosted submit is forbidden. Catalog restamp of gastownhall `registry.toml` is not that dest.
- `python scripts/check_pstack_delivery_evidence.py` is delivery evidence. It subprocesses dest standing `python scripts/check_pstack_dest_standing.py`, schema inventory, and `python pstack/scripts/apply_intent_change.py --source openspec/changes/archive/2026-09-02-pstack-mapping-gaps --validate-only`. The change name is the `--source` directory with a leading date prefix stripped. `--dest` defaults to the repository root. `--archive` merges into `openspec/specs/`. New payloads live under `openspec/changes/`. They do not live under `pstack/` or `docs/openspec-changes/`. It MUST NOT restamp. It MUST NOT wrap sling, inference-gate, or publish.
