...
- `gc pack registry publish --dry-run pstack/` is proven from this branch. `gc pack registry whoami` succeeds. Dry-run is not registry acceptance. Unscoped hosted submit from tommy waits on the scoped-name unit. Catalog restamp of gastownhall `registry.toml` is not that dest.
