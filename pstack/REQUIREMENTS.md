...
- Disposable live-city formula list, formula show, and agent listing when `GC_TEST_BIN` is set; no canonical city mutation. Formula sling of `pstack-poteto-mode` and `pstack-build` remains unproven.
- `python3 scripts/gascity_pack_inference_gate.py --pack pstack --setup-only --skip-inference-env-check` compiles `pstack-review` and `pstack-build` with `gc formula show` in the inference-gate city. That city is not formula sling. Formula sling of `pstack-poteto-mode` and `pstack-build` remains unproven.
