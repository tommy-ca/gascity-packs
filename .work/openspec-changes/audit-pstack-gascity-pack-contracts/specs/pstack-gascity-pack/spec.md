...
pack MUST NOT ship `pstack/intent/changes/`. Dest-env owns durable Gherkin.
`pstack/scripts/apply_intent_change.py` MUST require an external `--source`
and MUST refuse paths under `pstack/`.
...
- **AND** a later dest-env copy uses `--source` outside `pstack/`
